 ...
 *   menu bar rendering for non-default themes.
 */
#include "client.h"
...
/************************************************************************/
/*
 * ThemedMenuBar_Apply:  Convert all top-level menu bar items to
 *   owner-drawn so they render with the active theme's colors.  Sets the
 *   menu bar background brush via MENUINFO.  Safe to call multiple times;
 *   items already converted are skipped, so modules can call this after
 *   adding new top-level menus.  No-op if the default theme is active.
 */
void ThemedMenuBar_Apply(HMENU hMenu)
{
   if (!hMenu || !ThemeIsDark())
      return;
...
/************************************************************************/
/*
 * ThemedMenuBar_Remove:  Restore all owner-drawn menu bar items to
 *   standard string items and clear the background brush.
 */
void ThemedMenuBar_Remove(HMENU hMenu)
{
   if (!hMenu)
      return;
...
/************************************************************************/
/*
 * ThemedMenuBar_Destroy:  Remove owner-drawn state from the current
 *   menu and free the themed menu bar brushes.
 */
void ThemedMenuBar_Destroy(void)
{
   HMENU hMenu = GetMenu(hMain);
   if (hMenu)
      ThemedMenuBar_Remove(hMenu);
...
/************************************************************************/
/*
 * ThemedMenuBar_MeasureItem:  Returns true and fills the
 *   MEASUREITEMSTRUCT if the item is an owner-drawn menu bar item under
 *   a non-default theme.  Returns false if the item is not handled here.
 */
bool ThemedMenuBar_MeasureItem(MEASUREITEMSTRUCT *mis)
{
...
/************************************************************************/
/*
 * ThemedMenuBar_DrawItem:  Returns true and paints the item if it is an
 *   owner-drawn menu bar item under a non-default theme.  Returns false
 *   if the item is not handled here.
 */
bool ThemedMenuBar_DrawItem(DRAWITEMSTRUCT *dis)
{
