 ...
 *   portrait, stats) with the dark inventory texture in the dark theme.
 *   No-op for the default theme.  Drawn before the frame elements so the
 *   ornamental edges paint on top.
 */
...
void InterfaceDrawElements(HDC hdc)
{
  ...
  for (i=0; i < NUM_AUTO_ELEMENTS; i++)
  {
	  ...
	  // Skip silver stats frame corners in dark theme.  The sidebar
	  // is filled with the dark inventory texture so the silver pieces
  ...
  for (i=0; i < NUM_AUTO_REPEATERS; i++)
  {
	...
	// Skip silver stats edge repeaters in dark theme to avoid drawing
	// over the dark inventory sidebar fill.
	if (ThemeIsDark())
	{
